    ...
    const connectToMetamask = async () => {
      try {
        if (window.ethereum) {
          ...
          const contractInstance = new ethers.Contract(Constants.CONTRACT_ADDRESS, Constants.CONTRACT_ABI, signer);
          ...
          <tbody>
            {tasks.map((task, index) => (
              <tr key={index}>
                ...
                <td >
                {task.status === 0 ? <button className={styles.button} onClick={() => changeTaskStatus(index)}>Finish</button> : null}
